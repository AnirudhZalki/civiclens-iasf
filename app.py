...
import os, datetime, sqlite3, json, time, threading, csv, io
...
app.secret_key = 'civic_lens_secret_key_123'
...
def citizen():
    if request.method == 'POST':
        ...
        if best_dept['name'] == "Invalid":
            ...
            cursor.execute('''
                           INSERT INTO issues (image, address, assigned_dept, status, last_update, citizen_email,
                                               issue_id, created_date, latitude, longitude)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                           ''', (unique_filename, final_address, "Invalid", "Rejected",
                                 str(datetime.date.today()), citizen_email, generate_issue_id(),
                                 str(datetime.date.today()), latitude, longitude))
            conn.commit()
            conn.close()
            return render_template('citizen.html', error="❌ Not a valid image for civic issue reporting.")
        ...
        <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333;'>
            <div style='max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>
                ...
                <ul style='list-style-type: none; padding: 0;'>
                    ...
                    <li style='margin-bottom: 10px;'><strong>Coordinates:</strong> {latitude}, {longitude}</li>
        ...
        return render_template('submission_success.html', issue_uid=issue_uid, dept_name=best_dept['name'])
...
def update_day_progress(dept_id, issue_id):
    ...
    try:
        ...
        cursor.execute("SELECT citizen_email, issue_id, assigned_dept FROM issues WHERE id = ?", (issue_id,))
        citizen_email, issue_uid, dept_name = cursor.fetchone()

        if citizen_email:
            subject = f"Update on Issue #{issue_uid} - Day {day_number}"
            body = f"""
            Hello,

            This is an update regarding your reported issue #{issue_uid}.
            ...
            **Day {day_number} Update:**
            ...
            Civic Lens Team
            """
            # Send the email (HTML format is better, reusing the logic from /citizen)
            html_body = f"""
            <html>
            <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333;'>
                <div style='max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>
                    <h2 style='color: #2a5c5c;'>Issue Update: Day {day_number}</h2>
                    <hr style='border-color: #ddd;'>
                    <p>Hello,</p>
                    <p>There is a new update on your reported issue <strong>#{issue_uid}</strong>.</p>
                    
                    <div style='background-color: #f9f9f9; padding: 15px; border-left: 4px solid #38b2ac; margin: 20px 0;'>
                        <p><strong>Update for {datetime.date.today()}:</strong></p>
                        <p>"{description}"</p>
                    </div>

                    <p><strong>Department:</strong> {dept_name}<br>
                    <strong>Status:</strong> In Progress</p>

                    <p>You can track the full progress using your Issue ID on our <a href="#">tracking page</a>.</p>
                    
                    <hr style='border-color: #ddd;'>
                    <p style='font-size: 0.8em; color: #888; text-align: center;'>
                        Civic Lens - Building Better Communities
                    </p>
                </div>
            </body>
            </html>
            ...
            # Using threading to avoid blocking the response
            email_thread = threading.Thread(target=send_email, args=(citizen_email, subject, html_body, None, True))
            email_thread.start()
            print(f"Update email queued for {citizen_email}")
...
def resolve_issue(dept_id, issue_id):
    ...
    if request.method == 'POST':
        ...
        if resolution == 'resolved':
            ...
            cursor.execute('''
                           SELECT citizen_email, issue_id, assigned_dept
                           FROM issues
                           WHERE id = ?
                           ''', (issue_id,))
            citizen_email, issue_uid, dept_name = cursor.fetchone()
            ...
            if citizen_email:
                subject = f"Update on Issue #{issue_uid} - Extension Required"
                body = f"""
                Dear Citizen,

                We are writing to inform you that we need more time to resolve your reported issue #{issue_uid}.

                **Reason for Delay:**
                {extension_reason}

                **Additional Time Needed:** {extra_days} days.
                ...
                <body style='font-family: Arial, sans-serif; line-height: 1.6; color: #333;'>
                    <div style='max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;'>
                        <h2 style='color: #e53e3e;'>Issue Update: Extension Required</h2>
                        <hr style='border-color: #ddd;'>
                        <p>Dear Citizen,</p>
                        <p>This message is regarding your reported issue <strong>#{issue_uid}</strong>.</p>
                        ...
                        <div style='background-color: #fff5f5; padding: 15px; border-left: 4px solid #e53e3e; margin: 20px 0;'>
                            ...
                            <p>"{extension_reason}"</p>
                            <p><strong>Additional Time Estimated:</strong> {extra_days} days</p>
                        </div>

                        <p>We apologize for the delay and are working to resolve this as soon as possible.</p>
                ...
                email_thread = threading.Thread(target=send_email, args=(citizen_email, subject, html_body, None, True))
                email_thread.start()
                print(f"Extension email queued for {citizen_email}")
...
def admin_dashboard():
    ...
    # 2. Department Counts
    cursor.execute("SELECT assigned_dept, COUNT(*) FROM issues GROUP BY assigned_dept")
    dept_data = dict(cursor.fetchall())

    # 3. Last 7 Days Trends
    today = datetime.date.today()
    dates = [(today - datetime.timedelta(days=i)).strftime('%Y-%m-%d') for i in range(6, -1, -1)]
    trend_data = []
    for d in dates:
        cursor.execute("SELECT COUNT(*) FROM issues WHERE created_date = ?", (d,))
        trend_data.append(cursor.fetchone()[0])

    # 4. Recent Issues List
    cursor.execute("""
        SELECT issue_id, assigned_dept, status, created_date, address 
        FROM issues 
...
def admin_export():
    ...
    return Flask.response_class(
        output.read(),
        mimetype='text/csv',
        headers={"Content-disposition": "attachment; filename=civic_reports_export.csv"}
    )
